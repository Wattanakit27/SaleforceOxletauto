...
def update_release_date(request):
    ...
    # ★ ส.ค.69 — เลิก whitelist ตำแหน่งตายตัว: คอลัมน์ในชีตขยับทุกครั้งที่แอดมินแทรกช่องใหม่
    #   (เม.ย.69 แทรก 5 ช่อง → สถานะ 13->18 · เซ็น 14->19 · ปล่อย 21->27/28)
    #   ตอนนี้หน้าเว็บส่ง "ตำแหน่งจริงของแถวนั้น" มาให้ (statusCol/signCol/resultCol/releaseCol
    #   ที่อ่านจากหัวตาราง) → ตรวจแค่ว่าอยู่ในช่วงที่เป็นไปได้ + ตรวจ "ชนิดค่า" ตาม field ที่ส่งมา
    field = (body.get("field") or "").strip()
    if not (0 <= col <= 40):
        return JsonResponse({"error": "คอลัมน์ไม่อนุญาตให้แก้"}, status=400)
    is_status = (field == "status") or (not field and col == 13)   # ไม่ส่ง field = ไคลเอนต์เก่า
    if is_status:
