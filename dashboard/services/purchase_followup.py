...
ที่มาข้อมูล: ชีตจัดซื้อ (PURCHASE_SID) แท็บ "ขายรถจบออนไลน์ <เดือน>69"
  A วันที่ · C Code · D รุ่นรถ (พิมพ์เอง) · **S รถตามสูตร (ใช้ตัวนี้เทียบรุ่น)** · F เบอร์ติดต่อ · G ชื่อผู้ขาย
...
# ★ 25 ก.ย.69 — เพิ่ม S(18) "car" = **รถตามสูตร** ที่ทีมกรอกไว้แล้ว (วัดจริง: กรอก 375/379 = 98%)
#   เดิมเดาชื่อรุ่นเอาจาก D (คนพิมพ์อิสระ "MAZDA 2 ปี22 ขาว") ด้วยพจนานุกรม ได้ 90%
#   แต่ S เป็นคำตัดสินของคน → แม่นกว่า และทำสิ่งที่ตัวเดาทำไม่ได้:
        ...
        for tab in _months_to_read(now):
            ...
            for row in r.json().get("values", []):
                ...
                cases.append({
                    ...
                    "model_std": g("model_std"),      # S "car" = รถตามสูตรที่ทีมกรอกเอง
                })
    except Exception:
        return []
    return cases
