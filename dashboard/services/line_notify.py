...
def push_line_message(user_id: str, messages: list[dict], channel_token: str, timeout: int = 10,
    ...
    ★★ 24 ก.ย.69 — **ถอด "ตัวรับสำรอง" ออกแล้ว** (เจ้าของสั่ง *"การแจ้งเตือนจะยกเลิกการใช้
    [บัญชีเก่า] ไปเลย จะใช้เป็นบัญชี OxletautoGiveLead"*)

    ของเดิม: ส่งด้วยบัญชีตัวส่งไม่ผ่าน + ปลายทางเป็นคน (U…) = ลองซ้ำด้วย **บัญชีตัวรับ**
    ตั้งใจให้เป็นตัวกันข้อความหายระหว่างที่คนยังเพิ่มบอทใหม่ไม่ครบ
    **แต่ผลคือข้อความไปโผล่ในนามบัญชีที่เลิกใช้แล้ว** โดยคนรับไม่รู้ว่าทำไม
    (วัดจริง 24/09: ตารางเช็คชื่อที่เจ้าของขอ ส่งด้วยตัวส่งล้ม → ตัวสำรองส่งจาก OxletAuto)
