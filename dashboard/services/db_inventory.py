...
TABLES = {
    ...
    "dash_ads_daily": dict(
        name="ยอดโฆษณารายวัน (รวมทั้งบัญชี)",
        what="ผลรวมรายวันของโฆษณา Meta — เงินที่ใช้ · แชทที่เริ่ม · ลีด · คลิกลิงก์ · วิว · แสดงผล · "
             "ไม่เก็บ \"ต้นทุนต่อแชท\" เป็นคอลัมน์โดยตั้งใจ (ต้องคิดจากผลรวมหารผลรวม ไม่ใช่เฉลี่ยรายวัน)",
        pii=False, keep="สร้างใหม่ได้เสมอ (manage.py ads_rebuild)"),
    "dash_youtube_channel_snapshot": dict(
        name="ยอดช่อง YouTube (รายวัน)",
        what="ผู้ติดตาม · วิวรวมทั้งช่อง · จำนวนคลิป ของแต่ละช่อง ณ สิ้นวัน · "
             "ไม่ต้องให้เจ้าของช่องกดอนุญาต (ใช้ API key ใบเดียว)",
        pii=False, keep="เก็บถาวร"),
    "dash_youtube_video_snapshot": dict(
        name="ยอดคลิป YouTube (ทุกเที่ยงคืน)",
        what="ยอดสะสมของคลิปย้อนหลัง 90 วัน (วิว/ไลก์/คอมเมนต์) · แถว cron วันนี้ลบเมื่อวาน = ยอดรายวัน · "
             "YouTube ไม่ให้ยอดแชร์และดิสไลก์ จึงไม่มี 2 ช่องนั้น",
        pii=False, keep="เก็บถาวร"),
    "dash_youtube_raw": dict(
        name="ข้อมูลดิบจาก YouTube",
        what="คำตอบดิบทั้งก้อนจาก YouTube Data API — ไว้คิดตัวเลขใหม่ย้อนหลังถ้าสูตรเปลี่ยน",
        pii=False, keep="ลบเองเมื่อเกิน 90 วัน"),
    ...
    "dash_social_daily": dict(
        name="ยอดโซเชียลรายวัน (Facebook + TikTok)",
        what="ยอดที่ 'เพิ่มขึ้นจริงในวันนั้น' ของโพสต์/คลิปแต่ละชิ้น — 1 แถว/ชิ้น/วัน · "
             "คำนวณมาจากตาราง snapshot (ซึ่งเก็บยอดสะสม) เพื่อให้กรองช่วงวันที่ได้ตรงๆ · "
             "ลบทิ้งแล้วสร้างใหม่ได้เสมอด้วย manage.py social_rebuild · เป็นยอดรวม ไม่มีชื่อคน",
        pii=False, keep="สร้างใหม่ได้ (ต้นฉบับคือตาราง snapshot)"),
    "dash_meta_page_daily": dict(
        name="ยอดระดับเพจที่ Facebook รายงานเอง (รายวัน)",
        what="วิววิดีโอ/การมีส่วนร่วมทั้งเพจต่อวัน ตามที่ Meta สรุปให้ (ย้อนหลังได้ ~30 วัน) · "
             "ไว้เทียบว่ายอดที่เรารวมจากโพสต์เก็บครบไหม · เป็นยอดรวม ไม่มีชื่อคน",
        pii=False, keep="เก็บถาวร"),
    ...
    "dash_event_log": dict(
        ...
        what="ส่งอะไรออก LINE บ้าง สำเร็จ/ล้มเพราะอะไร · webhook ขาเข้าที่ผิดปกติ · "
             ...
             "★ ช่อง target คือปลายทางที่ส่ง = LINE user id ของพนักงาน หรือ group id",
        # ★ 26 ก.ย.69 แก้จาก pii=False — วัดจริงบนเซิร์ฟเวอร์: target เป็น LINE user id
        #   ของพนักงาน 464 แถว (ขึ้นต้น U ยาว 33) · ป้ายเดิมทำให้ตารางนี้ดูปลอดภัยเกินจริง
        pii=True, keep="เก็บ 90 วัน แล้วลบเอง"),
    ...
    # ---------- พนักงาน & ลูกค้า ----------
    # ★ 26 ก.ย.69 — 3 ตารางนี้เคยไม่มีคำอธิบาย จึงโผล่เป็นชื่อโมเดลดิบ
    #   และ **ไม่มีจุดส้มบอกว่ามีข้อมูลส่วนบุคคล** ทั้งที่ 2 ใน 3 มีเต็มๆ
    "checkout_employee": dict(
        name="ทะเบียนพนักงาน",
        what="ตัวคน 1 แถว/คน: ชื่อเล่น · ชื่อที่ตั้งใน LINE · ตำแหน่ง/ทีม · เวลาเข้างาน · "
             "วันหยุด · หมายเหตุ (ลา/สาย) · ต้องเช็คชื่อไหม · แจ้งเตือนไหม — "
             "เป็นที่เก็บ 'ทีม' ที่เดียวของทั้งระบบ (แดชบอร์ดขายอ่านทับจากที่นี่)",
        pii=True, keep="เก็บถาวร · คนลาออกให้ปิดใช้งาน ไม่ต้องลบ"),
    "checkout_checkin": dict(
        name="เช็คชื่อเข้างาน",
        what="1 แถวต่อ 'คน 1 วัน': มากี่โมง · ตรงเวลา/สาย · เวลาเข้างานของคนนั้น ณ วันนั้น · "
             "อ่านเวลาจากรูปหรือจากเวลาที่ส่ง · ที่อยู่ที่แนบมา · ผล OCR ดิบ",
        pii=True, keep="เก็บถาวร (เป็นประวัติการทำงาน)"),
    "checkout_customerneed": dict(
        name="ลูกค้าหารถอะไร",
        what="1 แถว = ความต้องการ 1 เรื่อง: รุ่นที่หา · ปี · งบ · ผ่อนไหวเดือนละ · ดาวน์ได้ · "
             "ชื่อลูกค้า + ช่องทางติดต่อ · ปิดเคสเพราะอะไร · ยังรอรถอยู่ไหม",
        pii=True, keep="ตามโปรไฟล์ลูกค้าที่ผูกไว้"),
