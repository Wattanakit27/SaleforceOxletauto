...
def dm_token() -> str:
    ...
    ระหว่างที่พนักงานยังเพิ่มบอทใหม่ไม่ครบ มี **ตัวกันข้อความหาย** อยู่ที่ `push_line_message()`
    (ส่งไม่ถึง → ลองซ้ำด้วยบัญชีตัวรับ + จดล็อกไว้ว่าใครยังไม่ได้เพิ่ม) — ดูที่นั่น
...
def followup_token() -> str:
    """★ 20 ก.ย.69 — **ข้อยกเว้นของนโยบาย: "ตามด่วน" ยังใช้บัญชีเดิม (ตัวรับ)**

    เจ้าของสั่งชัด: ทุกอย่างย้ายไปบอทตัวส่ง **ยกเว้น 2 ตัวนี้**
      · ตามด่วนรายเซลล์ (`ตามด่วน (cron ตามตารางส่ง)`)
      · ตามด่วนสรุปทีมให้แอดมิน (`ตามด่วน (สรุปทีมให้แอดมิน)`)
    เพราะเป็นแชท 1:1 ที่ทีมเพิ่มบัญชีเดิมเป็นเพื่อนไว้แล้ว **และส่งอยู่ได้ดีทุกวัน**
    (วัดจริง 7 วัน: สำเร็จ 112 + 72 ครั้ง ไม่เคยล้ม) — ย้ายไปบัญชีใหม่ = เสี่ยงหายทั้งทีมโดยไม่จำเป็น

    จะย้ายเมื่อไหร่ค่อยเปลี่ยนที่นี่ที่เดียว (ทุกจุดที่ส่งตามด่วนเรียกฟังก์ชันนี้)
    """
    return crm_token() or push_token()
