...
- **★ รื้อสเตป + บทบาท (ก.ค.69) — 16 สเตป/4 เฟส + ความด่วน + บังคับรูป + ถ่ายคอนเทนต์** ([constants.py](cars/constants.py)/[roles.py](cars/roles.py)/[models.py](cars/models.py) · migration **0010_restage_v2_priority** — ต้อง `migrate` ตอน deploy · remap สเตปเก่า→ใหม่ให้อัตโนมัติ):
  - **16 สเตป (4 เฟส · `STAGES`/`PHASES`)**: รับเข้า·รถรอถ่ายรูป | เช็คซ่อม·สั่งของ/รออะไหล่·งานเบาะ·อู่สีใน·อู่สีนอก·ติดฟิล์ม·ชงล้าง·รอตรวจรถขึ้นโชว์ | รถพร้อมขาย·จอง·จัดไฟแนนซ์·รอปิดการขาย | ตรวจรถรอปล่อย·ปล่อยรถ. **ปล่อยรถ = จบ (status=sold หลุดบอร์ด · ไม่มี "ขายแล้ว" เป็นสเตปแยก)** · แยกอู่สีเป็น ใน(`paint_in`)/นอก(`paint_out`) · เลิกจุด QC กลาง 3 จุดเดิม → รวมเป็น `qc_show`/`qc_release`
  - **ความด่วน (`Car.priority`)** = สีการ์ด เลือกมือ (แทนสีตามวันค้าง): ด่วนมาก/ด่วน/ปกติ/ไม่เร่ง/รอถ่ายรูป (`PRIORITY_COLOR`/`PRIORITY_NAME`) → `POST /track/api/set_priority`
  ...
  - **สิทธิ์สเตป (`STAGE_ROLES`/`FULL_ROLES={EXEC,REGIST,QC,PAINTIN}`)**: รับเข้า→จัดซื้อ · รถรอถ่ายรูป/เช็คซ่อม/ชงล้าง→โปรดักชัน · อะไหล่/งานเบาะ/ฟิล์ม→ช่าง · รอตรวจรถขึ้นโชว์→ฝ่ายล้างรถ (ล้างเสร็จส่ง QC) · **อู่สีใน/อู่สีนอก/ตรวจรถรอปล่อย→สิทธิ์เต็มเท่านั้น** (ฝ่ายทะเบียน/QC/อู่สีใน/ผู้บริหาร · อู่สีนอก=สถานะอู่ภายนอก ฝ่ายทะเบียนปล่อย/รับกลับ) · ขาย(show/reserve/finance/closing)+ปล่อยรถ→เซลล์. **เช็คซ่อม/ชงล้าง = คนหน้างาน(ช่าง/ล้าง)แค่ทำ/ตรวจดู ไม่กดสเตปเข้าเอง (โปรดักชันใส่ให้)** · แอดมิน=ไม่เปลี่ยนสเตป
