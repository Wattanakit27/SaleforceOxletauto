...
| เวลาปิดดีล (เดิม "ความเร็ว") | 10 | เคสปล่อย: เฉลี่ย 3 ช่วง (จอง→เซ็น ≤3วัน · เซ็น→ผล ≤3วัน · ผล→ปล่อย ≤1วัน) ×10 |
...
3. **🧹 ข้อมูลต้องเติม/แก้** (`renderDataQualityPanel()`) — dashboard ให้ admin ตามไปเติมข้อมูลในชีต · **กรองตามช่วงวันที่ (วันจอง) ทุกกลุ่ม ยกเว้นวันจองพัง**
   - **วันปล่อยว่าง**: status="ปล่อย" แต่ไม่มีวันปล่อย → กรองตามช่วง · กระทบคะแนนเวลาปิดดีล + การนับเดือน
   - **ปีผิด (ช่องอื่น)**: วันจองยังถูก แต่วันเซ็น/ผล/ปล่อย ปีนอก 2020–2035 → กรองตามช่วง (ด้วยวันจอง)
   - **วันจองพัง**: วันจองเองปีผิด (มักพิมพ์ 1969) → เคสนับเดือนเพี้ยน · **กรองตามช่วงไม่ได้ จึงแสดงเสมอ** (วันจองคือตัวที่ผิด)
