...
- **★ รื้อสเตป + บทบาท (ก.ค.69) — 16 สเตป/4 เฟส + ความด่วน + บังคับรูป + ถ่ายคอนเทนต์** ([constants.py](cars/constants.py)/[roles.py](cars/roles.py)/[models.py](cars/models.py) · migration **0010_restage_v2_priority** — ต้อง `migrate` ตอน deploy · remap สเตปเก่า→ใหม่ให้อัตโนมัติ):
  ...
  - **★ 📚 แถบแท็บ → sidebar ด้านซ้าย (ส.ค.69 · จอกว้าง ≥1100px)**: [globals.css](dashboard/static/dashboard/css/globals.css) — `#root:has(> #nav-slide)` เป็น grid 200px+เนื้อหา · `#nav-slide` แนวตั้ง sticky · จอแคบ/มือถือ = แถบนอนเดิม · `positionNavInd()` ([index.html](dashboard/templates/dashboard/index.html)) สไลด์แคปซูลแกน Y เพิ่ม (top/height) · **แก้ CSS ต้อง bump `?v=` ที่ link globals.css ใน index.html**
  ...
  - **สิทธิ์สเตป (`STAGE_ROLES` · ปรับ ส.ค.69 รอบ 2)**: **"รับเข้า" ไม่มีใครกดได้** (`NO_ONE_STAGES` — รถเกิดที่รับเข้าตอนสร้างอยู่แล้ว · จัดซื้อเลยไม่มีปุ่มสเตป) · **โปรดักชัน (3)** รถรอถ่ายรูป/เช็คซ่อม/รถพร้อมขาย (เอาชงล้างออก) · **ช่าง (3)** ซ่อมเสร็จรอตรวจ/อะไหล่/งานเบาะ (เอาฟิล์มออก) · **ฝ่ายล้างรถ (2)** รอ QC ตรวจ + รอเซลล์ตรวจ (ติ๊กแทน QC ชั่วคราว) · **เซลล์ (8)** **ตีกลับ QC** (=qc_show · ป้ายปุ่มเฉพาะเซลล์ผ่าน `STAGE_BUTTON_LABELS`/`stage_button_label()`)/พร้อมขาย/จอง/ไฟแนนซ์/**ตรวจขนส่ง**/ตรวจรถรอปล่อย/ปล่อยรถ/**ขายแล้ว** (ตัดรอปิดการขายออก) · **ฝ่ายทะเบียน = ทุกสเตปยกเว้น เบาะ/อะไหล่** (`REGIST_EXCLUDE` — ไม่ full สเตปแล้ว แต่ยัง `FULL_ROLES` ฝั่งจัดการ) · **รอปิดการขาย/รอตรวจสี/ฟิล์ม/ชงล้าง = ฝ่ายทะเบียน+สิทธิ์เต็มเท่านั้น** · `STAGE_FULL_ROLES={EXEC,QC,PAINTIN}` (สิทธิ์เต็มสเตป) · แอดมิน=ไม่เปลี่ยนสเตป
