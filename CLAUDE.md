...
- **ยังตอบกลับจากหน้านี้ไม่ได้** — LINE ไม่ส่งข้อความขาตอบกลับมา (ดูหัวข้อ provider/ขั้นที่ 2)
  หน้านี้เขียนบอกผู้ใช้ไว้ตรงๆ กันเข้าใจผิดว่าเห็นบทสนทนาครบ 2 ฝั่ง
...
### 🔁 เคสรับซื้อ/เทิร์นรถ — ย้ายจากชีตมา Postgres (17 ก.ย.69 · เจ้าของสั่ง)
*"เราไม่ได้ใช้ Sheet แล้วในการเก็บข้อมูล เราใช้ Postgres"*

workflow n8n **"ซื้อขายเทิร์นรถ"** อ่านข้อความเคสจากกลุ่ม LINE → parse → เดิม**เขียนลงชีต**
(ไฟล์ `ซื้อขายเทิร์นรถ` = `PURCHASE_SID` ตัวเดียวกับที่ `purchase_followup` อ่าน) → **ย้ายมาเขียน
ตาราง `dash_purchase_case` แทน**

- **`dashboard.PurchaseCase`** (`dash_purchase_case` · migration **0006**) — **1 แถว = 1 เคส ·
  คีย์ `code`** (OC-/SC-/TC-) → n8n ใช้ `ON CONFLICT (code)` upsert ส่งซ้ำก็ทับแถวเดิม ·
  **คอลัมน์เรียงตามชีตเดิม A–T** เพื่อให้คนที่เคยดูชีตอ่านออกทันที
- **📄 [deploy/n8n_tradein_postgres.json](deploy/n8n_tradein_postgres.json)** — 2 โหนด Postgres
  ก๊อปวางบน canvas (Ctrl+V) แทนโหนดชีต: **บันทึกเคส** (แทน append) · **เติมคอมเมนท์จัดซื้อ**
  (แทน `Get row → Merge Comments → update` **3 โหนด** — ทำต่อท้ายด้วย `" / "` ในคำสั่งเดียว
  จึงไม่มีจังหวะแข่งกันเขียน)
  - **★ n8n ห้ามทับช่องที่คนกรอกเอง**: `seq` (คันที่) · `car_category` (ประเภทรถ) · `comment` · `reason`
    · และค่าที่ parse ไม่ได้ (ว่าง) ก็ไม่ทับของเดิม — `coalesce(nullif(EXCLUDED.x,''), t.x)`
  - GRANT อยู่ใน [deploy/n8n_postgres_access.sql](deploy/n8n_postgres_access.sql) **ข้อ 6**
    (`SELECT, INSERT, UPDATE` · **ไม่ให้ DELETE** — เคสต้องไม่หายเพราะ workflow ยิงพลาด)
- **`purchase_followup.fetch_open_cases()` อ่านได้ 2 ที่** — สลับด้วย KV **`purchase_source`**
  (`{"source":"db"}`) · **ค่าเริ่มต้นยังเป็น `sheet`**
  - **⚠️★ ห้ามสลับอัตโนมัติตอนเห็นว่าตารางมีข้อมูล** (เวอร์ชันแรกผมทำแบบนั้น แล้วเห็นว่าอันตราย):
    ช่อง **"รับซื้อ/ไม่รับซื้อ" คือตัวปิดงาน** และทีม**ยังกรอกในชีต** → ถ้าสลับมาอ่าน DB
    ตอนที่ยังไม่มีที่ให้กรอก `decision` จะว่างตลอดกาล = **ทุกคนโดนทวงคันเดิมทุกวันไม่มีวันจบ**
  - **สลับได้ต่อเมื่อมีที่กรอกผลรับซื้อในระบบแล้วเท่านั้น** (ยังไม่ได้ทำ — งานถัดไป)
- **📄 [deploy/n8n_tradein_build_row.js](deploy/n8n_tradein_build_row.js)** = โค้ดเต็มของโหนด
  **`Build Sheet Row (Trade-in)`** (v20) — ก๊อปวางทับในช่อง Code ของโหนดนั้น ·
  v20 คืน **ชื่อฟิลด์แบบ flat** ให้โหนด Postgres อ่านตรงๆ และยังคง `values[]` ไว้
  เผื่อต่อโหนดชีตคู่ขนานช่วงเปลี่ยนผ่าน (เลิกใช้ชีตแล้วลบ `values` ทิ้งได้)
