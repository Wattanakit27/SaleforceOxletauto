...
- **รูปที่ 2 = กราฟแท่งการแข่งขันรายทีม `#rpt-shot-teams`** (ก.ค.69): aggregate `_rpt` ตาม `r.team` → ปล่อย(dn)/จอง(bk)/lead ต่อทีม เรียงปล่อยมากสุด · แถวแนวนอน สีทีม (`_tcol`) + เหรียญ 🥇🥈🥉 + ป้าย "จอง X · Lead Y" · ADMIN โชว์ "เทเลเซลล์" (`_tName`) · ซ่อนบนเว็บด้วย `#rpt-shot-teams-wrap` (height:0) เปิดก่อนแคปเหมือน #rpt-shot · width 920px. ส่งเข้า LINE เป็นรูปที่ 2 ต่อจากตาราง
