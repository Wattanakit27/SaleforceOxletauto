...
- **หน้าหัวหน้า `/checkout/`** (แท็บ "เบิก-คืนรถ" ในแดชบอร์ด · admin/ผู้บริหาร): KPI (รถออกอยู่/ไม่ครบ/รอยืนยัน) + ตารางทุกเคส · คอลัมน์ **งาน** (+⛽ ถ้าขอน้ำมัน) · ป้าย **"ค้าง N ชม."** แดงเมื่อเกิน 12 ชม. เหลืองเมื่อเกิน 4 (`overdueChip` · `outHours` จาก API)
...
- **★ โหมดเฝ้าดู (observe · ส.ค.69 — เจ้าของขอ "เอาบอทเข้ากลุ่มแล้วนั่งดูก่อน")**: เก็บข้อความในกลุ่ม + ผลที่ระบบ **"จะตีความ"** ไว้เทียบกัน โดย **ไม่สร้าง/แก้เคสใดๆ ทั้งสิ้น** → วัดความแม่นก่อนเปิดทำงานจริง
  ...
  - **★ ก.ย.69 — ชุดตัวอย่างจาก log จริง 10 วัน + คำสั่งวัดความแม่น** (เจ้าของส่งบทสนทนาจริงมาให้):
    ...
    - **วัดจริง (หลังแก้ parser)**: ข้อความที่คนพิมพ์ 160 → จับได้ **127 (79%)** · มั่นใจ 110 · เดา 17 · **จับทะเบียนได้แค่ 21/127 (17%)** เพราะคนส่งรูปแทนการพิมพ์ทะเบียน
    - **จับคู่ เบิก→คืน ได้ 45 คู่ · เบิกแล้วไม่เจอคืน 14 · "คืน" ที่ไม่มีเบิกนำหน้า 7** · เวลาใช้รถเฉลี่ย **2.2 ชม.** (นานสุด 8) → **`OVERDUE_HOURS`=12 สูงเกินจริง** (ในกลุ่มมีคนทวงตั้งแต่ ~2.5 ชม.)
  - **★ ก.ย.69 — parser แยก 4 ชนิด (เดิม 2)**: `out` เบิก · `in` คืน · **`fuel` ขอเบิกน้ำมัน** · **`fuel_done` ส่งรูปหลังเติม** (`KIND_LABEL` ใน [parser.py](checkout/parser.py) — หน้า observe ดึงป้ายจากตัวนี้ อย่า hardcode ซ้ำ)
  ...
  - **ช่องว่างที่ยังเหลือ (ต้องให้เจ้าของตัดสิน)**: ไม่มีหมวด "ไปดูรถ / รับรถเข้าใหม่ (จัดซื้อ)" ใน `PURPOSES` · ไม่มีแนวคิด **"ส่งต่อรถระหว่างคน"** ("พี่นวลใช้รถต่อ") ซึ่งทำให้จับคู่เบิก-คืนขาด · ข้อความสั้นมากที่สื่อความจากรูป ("1061" "595" "ขนส่งค่ะ") ยังจับไม่ได้
