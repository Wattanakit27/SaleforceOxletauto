...
| `/api/admin/line_group_name` | `admin_line_group_name` | admin POST `{id}`: ดึงชื่อกลุ่ม LINE จาก group id (LINE group summary API · บอทต้องอยู่ในกลุ่ม) → ปุ่ม "ตรวจชื่อ" ในพาเนลรายงาน (ยืนยันว่า id คือกลุ่มไหน · LINE ไม่มี API ลิสต์กลุ่มทั้งหมด) |
...
- **`capture_report_image()`**: Playwright login แดชบอร์ด (`/login/?bg=1` · **ต้อง `page.evaluate` เปิด `<details>` break-glass ก่อน fill** เพราะฟอร์มพับอยู่) → รอ `#rpt-card` → `.screenshot()` → เซฟ `MEDIA_ROOT/reports/report_<ts>.png` · device_scale=2 (คมชัด)
  ...
  - **`#rpt-card`** = id ที่เพิ่มให้การ์ดรายงาน (`_rpt` ใน [index.html](dashboard/templates/dashboard/index.html)) เพื่อให้ Playwright จับ
- **`send_report_to_line(target, caption)`**: capture → เซฟรูป → สร้าง URL `SITE_URL/media/reports/...` → LINE image message (`push_line_message`). **⚠️ LINE ต้องดึงรูปจาก URL https สาธารณะ → ได้จริงเฉพาะ prod** (localhost ส่งไม่ได้ · dev คืน error "ยังไม่มี URL https")
...
- **ความชัด**: `device_scale_factor=3` (รูปคม ~5000px · ~250KB พอดีลิมิต LINE preview 1MB) · ตาราง 13 คอลัมน์กว้าง → ย่อในแชทมือถือตัวเล็ก (แตะดูเต็มคม) · อยากอ่านง่ายในแชทต้องลดคอลัมน์/ทำเวอร์ชันมือถือ (ยังไม่ทำ)
- **ยืนยันกลุ่ม**: ปุ่ม "ตรวจชื่อ" (`admin_line_group_name` · `checkGroupName`) ดึงชื่อกลุ่มจาก id · **LINE ไม่มี API ลิสต์กลุ่ม** (บอทรู้ id เฉพาะตอนได้ event จากกลุ่ม → dropdown เลือกกลุ่มต้องเพิ่ม inbound webhook ก่อน · ยังไม่ทำ)
