...
| `/checkout/api/checkins` | `api_checkins` | **admin/ผู้บริหาร** GET `?date=YYYY-MM-DD` (ไม่ใส่ = วันนี้): เช็คชื่อเข้างานรายวัน — มาแล้ว (ตรงเวลา/สาย/อ่านเวลาไม่ออก) · **ยังไม่เช็คชื่อ** · **วันหยุดของคนนั้น** (ตัดคนที่ตรงกับช่อง "วันหยุด" ออก ไม่งั้นขึ้นค้างทั้งที่เขาหยุดจริง) → เมนู "เช็คชื่อเข้างาน" · ลิงก์ตรง `/dashboard/?panel=checkin` |
| `/checkout/api/employees` | `api_employees` | **admin/ผู้บริหาร** GET: ทะเบียนพนักงานในระบบ (ชื่อเล่น · ชื่อที่ตั้งใน LINE · ตำแหน่ง · **เวลาเข้างาน** · วันหยุด · ผูก LINE กี่บัญชี) · POST `{action:"save"\|"delete", …}` เพิ่ม/แก้/ลบ · **ไม่ส่ง LINE user id ของพนักงานออกมา** (บอกแค่จำนวนบัญชีที่ผูก) → เมนู "ทีม & สิทธิ์ → พนักงาน" |
...
- UI: เมนูสามขีด → **"ลูกค้า & ข้อมูล"** → **ฐานข้อมูล & ดาวน์โหลด** (`openDbPanel`/`renderDbPanel` · มีช่องค้นหา กรองทั้งตารางและคีย์ kv)
  _(ก.ย.69 ย้ายจากหมวด "ตรวจสอบ & Log" — เจ้าของหาไม่เจอ เพราะชื่อหมวดไม่ได้บอกว่ามีข้อมูลลูกค้า)_
...
- **3 ทาง**: เมนูสามขีด → **ลูกค้า & ข้อมูล** → **ฐานข้อมูล & ดาวน์โหลด** → ปุ่ม **⬇ ท้ายแถว** แต่ละตาราง (CSV) · ปุ่ม **⬇ ทั้งหมด (.zip)** ในพาเนล · และ
...
- **หน้าดู**: เมนู → **ตรวจสอบ & Log → เช็คชื่อเข้างาน** (`openCheckins()` · เลือกวันย้อนหลังได้) ·
  โชว์ 3 ก้อน: มาแล้ว / ยังไม่เช็คชื่อ / วันหยุดวันนี้ + ป้าย **"ยังไม่ผูกทะเบียน"** เมื่อจับคู่คนไม่ได้
...
- **★ รื้อสเตป + บทบาท (ก.ค.69) — 16 สเตป/4 เฟส + ความด่วน + บังคับรูป + ถ่ายคอนเทนต์** ([constants.py](cars/constants.py)/[roles.py](cars/roles.py)/[models.py](cars/models.py) · migration **0010_restage_v2_priority** — ต้อง `migrate` ตอน deploy · remap สเตปเก่า→ใหม่ให้อัตโนมัติ):
  ...
  - **★ ☰ แถบแท็บ → เมนูสามขีด (hamburger drawer · ส.ค.69)**: ซ่อนแท็บไว้ กดปุ่ม ☰ ในหัวเว็บ → แผงเลื่อนออกจากซ้าย + ฉากหลังมืด · **ใช้ทุกขนาดจอเหมือนกัน** (drawer ลอยทับ ไม่กินพื้นที่ → เนื้อหาได้ความกว้างเต็มตลอด) · _(เดิม ส.ค.69 ต้นเดือนเป็น sidebar ติดซ้ายถาวรบนจอ ≥1100px — เปลี่ยนตามที่ผู้ใช้ขอ)_
    ...
    - **★★ ก.ย.69 — ทุกหมวดใน drawer "พับได้" (accordion) + จัดหมวดใหม่** (เจ้าของแจ้ง *"สไลด์เมนูเยอะเกินไปแล้ว"*)
      ...
      - **หมวดใหม่ 4 หมวด**: **ลูกค้า & ข้อมูล** (แชทลูกค้า/กลุ่ม LINE · ฐานข้อมูล & ดาวน์โหลด ·
