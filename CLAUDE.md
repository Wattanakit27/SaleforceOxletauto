...
| `/api/admin/update_release_date` | `update_release_date` | admin POST: inline edit "วันปล่อย" เขียนกลับชีตยอดขายตรงเซลล์ — body `{tab,row,col(21\|23),value}` → `update_release_date()` PUT cell · ใช้จากหน้ารายละเอียดเคส (`saveReleaseDate`) |
...
- **เฟส 2 (ทำแล้ว — TEST MODE)**: `build_followup_messages()` ([fetch_dashboard.py](dashboard/services/fetch_dashboard.py)) = mirror `followUrgency`+cadence+ดีลค้าง เป็น Python (pass เดียว group ตาม seller · ไม่เรียก `fetch_seller_stats` ต่อคน) → ข้อความธรรมดา top N + ลิงก์ `/s/`. `cron_tick` ส่ง **09:00 + 13:00** (BKK) ผ่าน `_FU_TIMES`. **`_FU_TEST_TARGET`** (ใน cron_tick) ตั้งไว้ = ส่งเข้าไลน์แอดมินทดสอบ · ตั้ง `""` = ส่งเซลล์จริงแต่ละคน. กรองเฉพาะเคสใหม่ (เดือนนี้ + 1 สัปดาห์ท้ายเดือนก่อน) · dedup ดีลค้าง · ใช้ canonical URL `_FU_BASE`. ยังไม่มี anti-spam Supabase (อาศัย time-match HH:MM เหมือน schedule เดิม)
...
3. **🔔 กระดิ่งแจ้งเตือน (header, admin)** — `computeAdminAlerts()` + `openAlertsPanel()` (ข้างเมนูจัดการ มี badge จำนวน) · **แทน panel "ข้อมูลต้องเติม/แก้" เดิม** (`renderDataQualityPanel` ยังอยู่ในโค้ดแต่ไม่ render แล้ว — ตัดออกจาก LEAD tab มิ.ย.69)
   ...
   - **✏️ Inline edit วันปล่อย** (แอดมินเท่านั้น) — ในหน้ารายละเอียดเคส (`openBookingDetail` → `_relEdit`/`saveReleaseDate`) มีช่องกรอกวันปล่อย + ปุ่มบันทึก → POST `/api/admin/update_release_date` → เขียนกลับชีตตรงเซลล์. ตำแหน่งมาจาก `bookingCases[].sheetTab`/`sheetRow`/`releaseCol` ที่ `fetch_sales_by_month_tabs` แนบไว้ (**col 28=tab name, col 29=แถวในชีต 1-based** ต่อท้าย flattened row · `releaseCol`=23(X พ.ค.+)/21(V เดือนก่อน) จาก `_release_col(r)`). เขียนแล้ว optimistic อัปเดต local (mirror ตามทันรอบ sync ถัดไป). ถ้า `sheetTab` ว่าง (cache เก่า/failsafe) → แสดง read-only
