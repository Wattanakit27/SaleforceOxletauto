...
**★ ย้าย "รายชื่อ/ชื่อเล่น" จากชีตมา Postgres (17 ก.ย.69 · เจ้าของสั่ง)**
- **📄 [deploy/n8n_members_postgres.json](deploy/n8n_members_postgres.json)** — 2 โหนด ก๊อปวางบน canvas
  แทนโหนดชีต **`Get Members (Master)`** และ **`Upsert Member (Master)`**
  (แท็บ `รายชื่อสมาชิกกลุ่ม` ของไฟล์ `ซื้อขายเทิร์นรถ`)
...
- **⚠️ โหนดเขียนห้ามแตะ `nickname` / `is_employee` / `employee_id`** — 3 ช่องนี้ระบบเราจับคู่เอง
  ถ้า n8n เขียนทับด้วยค่าว่าง **คนที่จับคู่ไว้แล้วจะถูกรีเซ็ตกลับเป็น "ลูกค้า" ทุกครั้งที่พิมพ์**
  · คำสั่งเขียนแค่ `display_name` / `group_id` / `last_seen` · GRANT อยู่ใน
  [n8n_postgres_access.sql](deploy/n8n_postgres_access.sql) **ข้อ 6**
