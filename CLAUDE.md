...
- **★ รื้อสเตป + บทบาท (ก.ค.69) — 16 สเตป/4 เฟส + ความด่วน + บังคับรูป + ถ่ายคอนเทนต์** ([constants.py](cars/constants.py)/[roles.py](cars/roles.py)/[models.py](cars/models.py) · migration **0010_restage_v2_priority** — ต้อง `migrate` ตอน deploy · remap สเตปเก่า→ใหม่ให้อัตโนมัติ):
  - **17 สเตป (4 เฟส · `STAGES`/`PHASES`)**: รับเข้า·รถรอถ่ายรูป | เช็คซ่อม·**ซ่อมเสร็จรอตรวจ**·สั่งของ/รออะไหล่·งานเบาะ·อู่สีใน·อู่สีนอก·ติดฟิล์ม·ชงล้าง·รอตรวจรถขึ้นโชว์ | รถพร้อมขาย·จอง·จัดไฟแนนซ์·รอปิดการขาย | ตรวจรถรอปล่อย·ปล่อยรถ. **ปล่อยรถ = จบ (status=sold หลุดบอร์ด · ไม่มี "ขายแล้ว" เป็นสเตปแยก)** · แยกอู่สีเป็น ใน(`paint_in`)/นอก(`paint_out`) · เลิกจุด QC กลาง 3 จุดเดิม → รวมเป็น `qc_show`/`qc_release`
  ...
  - **ความด่วน (`Car.priority`)** = สีการ์ด เลือกมือ (แทนสีตามวันค้าง): ด่วนมาก/ด่วน/ปกติ/ไม่เร่ง/**ยังไม่ได้ถ่ายรูป** (`PRIORITY_COLOR`/`PRIORITY_NAME`) → `POST /track/api/set_priority` (**`csrf_exempt`** — บอร์ดถูกฝัง iframe ในหน้าเซลล์ที่ไม่มี CSRF token · ไม่ใส่ = ฝ่ายทะเบียน/คนอื่นกดความด่วนไม่ได้ 403)
    - **★ ธง "ยังไม่ได้ถ่ายรูป" (ส.ค.69)** = รถรับเข้าแล้วแต่ยังถ่ายรูปไม่ได้ → **ติดธงนี้แล้วย้ายไปสเตปอื่นได้เลย** (ซ่อม/สี/ล้าง) ไม่ต้องค้างในคิวถ่ายรูป · ธงติดตามรถไปทุกสเตป
  ...
  - **สิทธิ์สเตป (`STAGE_ROLES`/`FULL_ROLES={EXEC,REGIST,QC,PAINTIN}`) — ปรับตามที่ประชุม ส.ค.69**: รับเข้า→จัดซื้อ · **โปรดักชัน (4)** รถรอถ่ายรูป/เช็คซ่อม/ชงล้าง/**รถพร้อมขาย** · **ช่าง (4)** **ซ่อมเสร็จรอตรวจ**/อะไหล่/งานเบาะ/ฟิล์ม · **ฝ่ายล้างรถ (1)** รอตรวจรถขึ้นโชว์ (ล้างเสร็จส่ง QC) · **เซลล์ (7)** **รอตรวจรถขึ้นโชว์ (ส่งกลับ QC)**/พร้อมขาย/จอง/ไฟแนนซ์/รอปิดการขาย/**ตรวจรถรอปล่อย**/ปล่อยรถ · **อู่สีใน/อู่สีนอก→สิทธิ์เต็มเท่านั้น** (อู่สีนอก=สถานะอู่ภายนอก ฝ่ายทะเบียนปล่อย/รับกลับ) · **เช็คซ่อม/ชงล้าง = โปรดักชันเป็นคนใส่** (ช่าง/ฝ่ายล้างลงมือทำ แต่ไม่กดสเตปเข้าเอง) · แอดมิน=ไม่เปลี่ยนสเตป
