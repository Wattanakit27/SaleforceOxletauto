...
- **★ เคสจบตามไฟแนนซ์ (ส.ค.69)**: ผู้ใช้เพิ่มคอลัมน์ **"ชำระแบบ"** (KK/KL/TTB/AY/NISSAN/เงินสด...) ในฝั่ง "จบ" (บล็อกขวา) ของแท็บ **จอง/จบ** ชีตนับลีด → `fetch_finance_by_month_tabs()` ([google_sheets.py](dashboard/services/google_sheets.py)) อ่านแบบ **header-based** (หา "ชำระแบบ" ใน 5 แถวแรก + คอลัมน์สถานะ "ปิด" ทางขวา ≤8 ช่อง · เจอสถานะ = นับเฉพาะแถวมีคำ "ปิด" · ไม่เจอ = นับทุกแถวที่ชำระแบบไม่ว่าง) → `data["financeSummary"]` = `{เดือน:{ไฟแนนซ์:จำนวน}}` (เดือนตามแท็บ · best-effort error={}) · โชว์: **ตารางท้ายแท็บ "สถานะจองปล่อย"** (การ์ด `#fin-card` ใน `renderBookings` · กรองเดือนตามช่วงวันที่) + **รูปที่ 3 ของรายงาน LINE `#rpt-shot-fin`** (สร้างเฉพาะมีข้อมูล · ไม่มี = target โดน skip เอง)
- **★ เปลี่ยนชื่อแท็บ 'b' (ส.ค.69)**: "ติดตามสถานะจอง" → **"สถานะจองปล่อย"**
- **★ ขยายฟอนต์รูปรายงาน (ส.ค.69)**: `#rpt-shot` เลขหลัก 28→36 รอง 24→30 ชื่อเซลล์ 21→25 หัว 16-17→18-20 กล่อง 92→110/58→72 · `#rpt-shot-teams` ตาราง 15→19 หัว 14-15→17-18 — กว้างรวมยังไม่เกิน 1560/1120px เรียงแถวเหมือนเดิม
...
- **auth = Django auth + Group** (**10 บทบาท** · [cars/roles.py](cars/roles.py)): **สิทธิ์เต็ม (ทุกสเตป + เพิ่ม/แก้/ลบรถ + จัดการผู้ใช้)** = Executive/Registration/QC/PaintIn · **เฉพาะสเตปตัวเอง** = Purchasing/Production/Technician/CarWash/Sales · **Admin** = แก้ข้อมูลรถ + จัดการผู้ใช้ (ไม่เปลี่ยนสเตป). **(ก.ค.69: +Production/QC/PaintIn · ยุบ Vendor เข้า Registration)** — ตารางสิทธิ์เต็มดูหัวข้อ "รื้อสเตป+บทบาท" ด้านล่าง
- **★ รื้อสเตป + บทบาท (ก.ค.69) — 16 สเตป/4 เฟส + ความด่วน + บังคับรูป + ถ่ายคอนเทนต์** ([constants.py](cars/constants.py)/[roles.py](cars/roles.py)/[models.py](cars/models.py) · migration **0010_restage_v2_priority** — ต้อง `migrate` ตอน deploy · remap สเตปเก่า→ใหม่ให้อัตโนมัติ):
  - **18 สเตป (4 เฟส · `STAGES`/`PHASES`)**: รับเข้า·รถรอถ่ายรูป | เช็คซ่อม·**ซ่อมเสร็จรอตรวจ**·สั่งของ/รออะไหล่·งานเบาะ·อู่สีใน·อู่สีนอก·ติดฟิล์ม·ชงล้าง·**รอ QC ตรวจ** | **รอเซลล์ตรวจรถขึ้นโชว์**·รถพร้อมขาย·จอง·จัดไฟแนนซ์·รอปิดการขาย | ตรวจรถรอปล่อย·ปล่อยรถ. **ปล่อยรถ = จบ (status=sold หลุดบอร์ด · ไม่มี "ขายแล้ว" เป็นสเตปแยก)** · แยกอู่สีเป็น ใน(`paint_in`)/นอก(`paint_out`) · เลิกจุด QC กลาง 3 จุดเดิม → รวมเป็น `qc_show`/`qc_release`
  ...
  - **★ ธงงานค้าง (`CAR_FLAGS` · `Car.need_photo`/`need_content` · ส.ค.69 · migration 0013)** — **แยกจากความด่วน · ติ๊กได้หลายอันพร้อมกัน · ใช้ร่วมกับความด่วนได้**:
    ...
    - **ปุ่ม "ถ่ายคอนเทนต์" (`api_content_shoot`) ปลดธง `need_content` ให้เอง** — ไม่ต้องมากดออกซ้ำ
    ...
    - **★ การ์ดบนบอร์ดเรียงตามความด่วนก่อน (ส.ค.69)**: `dashboard` view ([views.py](cars/views.py) `board_cols`) เรียง `(ลำดับความด่วน, stage_since)` — ด่วนมาก (แดง) บนสุด ไล่ลงมา · ความด่วนเท่ากันเอาค้างนานสุดขึ้นก่อน (เดิมเรียงแค่วันค้าง การ์ดแดงจมอยู่ล่าง)
    - **เพิ่มธงใหม่**: แก้ `CAR_FLAGS` ([constants.py](cars/constants.py)) + เพิ่มฟิลด์ใน [models.py](cars/models.py) + `FLAG_DEFS` ใน [dashboard.html](templates/dashboard.html) ให้ตรงกัน
  ...
  - **สิทธิ์สเตป (`STAGE_ROLES`/`FULL_ROLES={EXEC,REGIST,QC,PAINTIN}`) — ปรับตามที่ประชุม ส.ค.69**: รับเข้า→จัดซื้อ · **โปรดักชัน (4)** รถรอถ่ายรูป/เช็คซ่อม/ชงล้าง/**รถพร้อมขาย** · **ช่าง (4)** **ซ่อมเสร็จรอตรวจ**/อะไหล่/งานเบาะ/ฟิล์ม · **ฝ่ายล้างรถ (2)** รอ QC ตรวจ (ล้างเสร็จส่งตรวจ) + **รอเซลล์ตรวจรถขึ้นโชว์ (ติ๊กแทน QC ชั่วคราว)** · **เซลล์ (7)** **รอ QC ตรวจ (ส่งกลับ QC)**/พร้อมขาย/จอง/ไฟแนนซ์/รอปิดการขาย/**ตรวจรถรอปล่อย**/ปล่อยรถ · **อู่สีใน/อู่สีนอก→สิทธิ์เต็มเท่านั้น** (อู่สีนอก=สถานะอู่ภายนอก ฝ่ายทะเบียนปล่อย/รับกลับ) · **เช็คซ่อม/ชงล้าง = โปรดักชันเป็นคนใส่** (ช่าง/ฝ่ายล้างลงมือทำ แต่ไม่กดสเตปเข้าเอง) · แอดมิน=ไม่เปลี่ยนสเตป
