...
- **แคป "เวอร์ชันย่อ" `#rpt-shot` (ไม่ใช่ตารางเต็ม 13 คอลัมน์)**: ตารางเต็มกว้างเกิน อ่านยากบนมือถือ → ทำ **`#rpt-shot`** ใน `renderOverview` (reuse `_rpt` เดียวกัน) = **6 คอลัมน์ (จอง/รอผล/รอปล่อย/ปล่อย/Lead/Conv%) · ตัวใหญ่ · แนวตั้ง · สี light คงที่** อ่านง่ายในแชท. **ซ่อนบนเว็บ** ด้วย wrapper `#rpt-shot-wrap` (`height:0;overflow:hidden`) → capture `page.evaluate` เปิดให้เห็น (`height:auto`) ก่อน screenshot · **#rpt-shot ต้อง `width:900px` เอง** (ไม่งั้น block เต็มความกว้าง → รูปมีที่ว่างขวา) · fallback `#rpt-card` ถ้าไม่มี
- **ความชัด**: `device_scale_factor=3` (รูปคม ~2700px กว้าง · ~330KB พอดีลิมิต LINE preview 1MB)
