...
- **deploy บน VPS ต้องลง Chromium**: `pip install playwright` + **`playwright install chromium --with-deps`** (lib ระบบ + ฟอนต์ไทย · Chromium launch ใช้ `--no-sandbox`) · ไม่ลง = แคปไม่ได้ (คืน None ไม่พังระบบ)
