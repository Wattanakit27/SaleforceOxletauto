...
- **หน้าหัวหน้า `/checkout/`** (แท็บ "เบิก-คืนรถ" ในแดชบอร์ด · admin/ผู้บริหาร): KPI (รถออกอยู่/ไม่ครบ/รอยืนยัน) + ตารางทุกเคส · คอลัมน์ **งาน** (+⛽ ถ้าขอน้ำมัน) · ป้าย **"ค้าง N ชม."** (`overdueChip` · `outHours` จาก API)
...
- **★ โหมดเฝ้าดู (observe · ส.ค.69 — เจ้าของขอ "เอาบอทเข้ากลุ่มแล้วนั่งดูก่อน")**: เก็บข้อความในกลุ่ม + ผลที่ระบบ **"จะตีความ"** ไว้เทียบกัน โดย **ไม่สร้าง/แก้เคสใดๆ ทั้งสิ้น** → วัดความแม่นก่อนเปิดทำงานจริง
  - **[checkout/parser.py](checkout/parser.py)** `parse(text)` → `{kind: out|in, plate, purpose, fuel, cancel, confidence, why}` · กติกาสังเคราะห์จากข้อความจริงในกลุ่ม (เบิกรถ/นำรถไป/เอารถ… · คืนรถ/คืน · เบิกน้ำมัน · ยกเลิก)
    - **⚠️ อย่าใส่คำสั้นในลิสต์คำใบ้** — เคยใส่ `"รับ"` แล้วไปโดน **"ครับ"** ที่ต่อท้ายเกือบทุกประโยค → "คืนรถครับ" กลายเป็นงานทั่วไป · เดาประเภทงานเฉพาะตอน `kind=="out"` (ตอนคืนข้อความสั้นมาก เดาไปก็มั่ว)
    - จับทะเบียนจากเลข 3-4 ตัวที่คนพิมพ์ (`เบิก5655`) → หา `Car.plate__endswith` · **ตัดลิงก์แผนที่ทิ้งก่อน** ไม่งั้นเลขพิกัดกลายเป็นทะเบียน
  - **โมเดล `GroupMessage`** (migration 0003) เก็บข้อความดิบ + ผล parse + `human_verdict` (คนกดถูก/ผิด) → คำนวณ **% ความแม่น** ได้จริง
  - **หน้า `/checkout/observe/`** — ตาราง "ข้อความจริง vs ระบบตีความว่าอะไร" + ปุ่มถูก/ผิด + ตัวกรอง (จับได้/ไม่มั่นใจ/จับไม่ได้/คนบอกว่าผิด) · KPI: เก็บได้กี่ข้อความ · จับได้กี่อัน · ความแม่นกี่ %
  - **★ ก.ย.69 — หน้าเฝ้าดูตั้งกลุ่มปลายทางได้เลย**: `api_observe` ส่ง `config.groupId` + `groups` (จาก KVStore `line_groups` = กลุ่มที่บอทจำได้) + `lineToken` → หน้าเว็บมี **dropdown เลือกกลุ่มตามชื่อ** (ไม่ต้องไปหา group id เอง) + เติมค่าที่ตั้งไว้ให้เห็น + เตือนเมื่อ **ยังไม่ตั้งกลุ่ม / ไม่มี LINE token**
  - **เปิด/ปิดที่ KVStore `checkout_line_config` `{observe: true}`** (ติ๊กในหน้านั้นได้เลย) · **ปิดอยู่ = ไม่เก็บอะไรเลย**
  - hook อยู่ที่ `dashboard.views._observe_record()` → เรียกจาก `line_webhook` · **best-effort** (checkout ล่ม/ปิดโหมด = ไม่กระทบ webhook เดิม)
  - วัดกับข้อความจริง 18 ข้อความจาก log: **จับได้ 14** (มั่นใจ 8 · เดา 6) · ที่ไม่จับ 4 = ข้อความที่ไม่เกี่ยวจริงๆ ("รับทราบค่ะ" "ส่งรูปไม่ครบค่ะ")
  - **★ ก.ย.69 — ชุดตัวอย่างจาก log จริง 10 วัน + คำสั่งวัดความแม่น** (เจ้าของส่งบทสนทนาจริงมาให้):
    - ไฟล์ [checkout/samples/group_log_sep69.txt](checkout/samples/group_log_sep69.txt) = 403 ข้อความ (31 ส.ค. – 9 ก.ย.69) · รูปแบบ `HH:MM <ชื่อ> <ข้อความ>` คั่นวันด้วย `YYYY.MM.DD`
    - `python manage.py seed_group_log [--dry-run] [--clear] [--out ไฟล์]` ([seed_group_log.py](checkout/management/commands/seed_group_log.py)) — ป้อนเข้า `GroupMessage` (group id **`SAMPLE_LOG`** · หน้า observe ติดป้าย "ตัวอย่าง") + พิมพ์รายงานความแม่น: จับได้/พลาด/เดา · จับคู่ เบิก→คืน ต่อคน · เวลาใช้รถ
    - **⚠️ console ไทยเพี้ยน (cp874) → ใช้ `--out` เขียนไฟล์ UTF-8 แทนการพิมพ์ลงจอ**
    - **วัดจริง (หลังแก้ parser 2 รอบ)**: ข้อความที่คนพิมพ์ 160 → จับได้ **135 (84%)** · มั่นใจ 110 · เดา 25 · **จับทะเบียนได้แค่ 24/135 (18%)** เพราะคนส่งรูปแทนการพิมพ์ทะเบียน
    - **จับคู่ เบิก→คืน ได้ 49 คู่ · เบิกแล้วไม่เจอคืน 15 · "คืน" ที่ไม่มีเบิกนำหน้า 3** · เวลาใช้รถเฉลี่ย **2.2 ชม.** (นานสุด 8) → เป็นที่มาของ `OVERDUE_HOURS`=4
  - **★ ก.ย.69 — parser แยก 5 ชนิด (เดิม 2)**: `out` เบิก · `in` คืน · **`fuel` ขอเบิกน้ำมัน** · **`fuel_done` ส่งรูปหลังเติม** · **`plate_only` อ้างถึงรถ** (`KIND_LABEL` ใน [parser.py](checkout/parser.py) — หน้า observe ดึงป้ายจากตัวนี้ อย่า hardcode ซ้ำ)
    - **บั๊กที่แก้**: คำว่า **"เบิก"** ทำให้ **"ขอเบิกน้ำมัน..." 12 ข้อความกลายเป็น "รถออกนอกลาน"** ทั้งที่บางครั้งรถจอดอยู่ในลาน ("เบิกน้ำมันเติมขยับรถครับ") → ถ้าไม่มีคำเบิกรถแบบชัด (`_OUT_STRONG`) = `fuel` ไม่ใช่ `out`
    - เพิ่ม: เขียนย่อไม่มีคำว่า "รถ" (`เอา 3414 กลับชลบุรี` · `นำ3413กลับชลบุรี` — `_SHORT_TAKE_RE`) · พิมพ์ผิด `คีน` = คืน · `ยืมรถ`/`รับรถ` · กันประโยคคำถาม (`ใคร…ไหม`) และสั่งงานล่วงหน้า (`พรุ่งนี้`) ไม่ให้นับเป็นการเบิก
    - **★ ทางเดาสุดท้าย (ก.ย.69 รอบ 2) — ข้อความที่สื่อความจาก "รูปที่ส่งคู่กัน"**: ตัด @mention + คำลงท้าย (`_MENTION`/`_POLITE`) แล้วดูแก่นข้อความ
      - **สั้น ≤24 ตัว + มีแต่ปลายทาง/งาน** (`ขนส่งค่ะ` · `ไปขนส่ง` · `กลับชลบุรี` · `ไปรับพี่โอ๊ดครับ` — `_SHORT_TASK`) → `out` **แต่ `confidence=low` เสมอ** ให้คนตรวจ
      - **พิมพ์แต่เลขทะเบียน** (`1061` · `595` · `HRV6149` — `_PLATE_ONLY_RE`) → **`plate_only`** ไม่เดาว่าเบิกหรือคืน (ตอบผิดครึ่งหนึ่งแน่ๆ) แต่เก็บเลขทะเบียนไว้ให้
      - **`_REQUEST` (`หน่อย`/`รบกวน`) ใช้เฉพาะทางเดาย่อนี้** — "@ART. ไปส่งหน่อยครับ" = สั่งคนอื่น ไม่ใช่ตัวเองเบิก · **อย่าเอาไปใส่ `_QUESTION`** ไม่งั้น "เบิกรถ...หน่อยครับ" จะหลุด
  - **ช่องว่างที่ยังเหลือ**: ไม่มีแนวคิด **"ส่งต่อรถระหว่างคน"** ("เบิกรถ...แล้วพี่นวลใช้ไปขนส่งต่อ" · "พี่นวลใช้รถต่อ") → คนเบิกไม่ใช่คนคืน ทำให้จับคู่ขาด (ส่วนหนึ่งของ 15/3 ที่ค้าง) · "เปลี่ยนคันครับ" (สลับคันกลางทาง) ตั้ง `cancel=True` แต่ยังไม่มี kind
