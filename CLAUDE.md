...
2. **ไอดีออกต่อ provider** — แท็กต้องใช้ไอดี **ฝั่งบัญชีที่กำลังส่ง** (`_mention_ids` กรองด้วย
   `LineProfile.channel`) · ไอดีผิดฝั่ง = LINE ตอบ 400 แบบไม่บอกสาเหตุ ·
   คำสั่งเตือนให้ล่วงหน้าถ้าคนที่จะส่งหาไม่มีไอดีของบัญชีนั้น
...
- **ยังไม่ผูก cron** — ตั้งใจให้ทดสอบด้วยมือก่อน ค่อยเรียกจาก `cron_tick` ทีหลัง
